...
jQuery(function($) {
	...
	mobileToggleClick();
	onePageNavigation();
	// siteIstotope();  -> handled in portfolio.js, after Supabase data is injected
	portfolioItemClick();
	owlCarouselPlugin();
	floatingLabel();
	scrollWindow();
	counter();
	jarallaxPlugin();
	// contactForm();   -> handled in portfolio.js (Web3Forms, no PHP backend)
	stickyFillPlugin();
	animateReveal();
	...
	$('.section-counter').waypoint( function( direction ) {
		...
		if( direction === 'down' && !$(this.element).hasClass('ftco-animated') ) {
			...
			$(this.element).find('.number-counter').each(function(){
				var $this = $(this),
					rawNum = $this.attr('data-number') || $this.data('number'),
				...
				var isFloat = String(rawNum).indexOf('.') !== -1;
				if (isFloat) {
